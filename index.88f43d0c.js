...
})({"62ocA":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "e69dd28788f43d0c";
...
},{}],"9Qpox":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "loadTeacherViewBaseStatus", ()=>loadTeacherViewBaseStatus);
...
let alertMessages = {
    [_errorCodes.OK]: "Se creo correctamente",
    [_errorCodes.CourseNotFound]: "No existe el curso",
    [_errorCodes.HomeworkNotFound]: "No existe la tarea",
    [_errorCodes.DeadlineAlreadyPassed]: "La fecha final ya paso",
    [_errorCodes.DeadlineCantBeLowerThanInit]: "La fecha inicial no puede ser mayor a la final"
};
...
createHmwkForm.addEventListener("submit", (event)=>{
    ...
    if (status == _errorCodes.OK) loadListByDates();
});
...
noNumberFields.forEach((noNumberField)=>{
    noNumberField.addEventListener("input", (event)=>{
        ...
        if (isNaN(noNumberField.value[currentLength]) == false && noNumberField.value[currentLength] != " ") noNumberField.value = noNumberField.value.slice(0, currentLength);
    });
});
function getCourseHomeworksFromAllStudents() {
    let answerSet = new Set();
    let courseName = coursesController.getCourseByName((0, _teacherLoginManager.getProfessorName)());
    answerSet.add(courseName);
    studentsInCourse = (0, _studentListJs.getStudentsInCourse)(courseName);
    let OtherCourses = (0, _studentListJs.getCoursesFromAllStudentsWithinACourse)(courseName);
    return new Set([
        ...answerSet,
        ...OtherCourses
    ]);
...
function loadDateContainer(homeworksArray, date) {
    ...
    addPropsToElement(dateContainer, {
        "id": "divFechaDocente" + date
    });
    addPropsToElement(dateTittleDiv, {
        "class": "divFecha" + date
    }, date + "==>");
    addElementsToFather(dateContainer, dateTittleDiv);
    for(let i = 0; i < homeworksArray.length; i++)addElementsToFather(dateContainer, createHomeworkItem(homeworksArray[i]));
...
function createHomeworkItem(homework) {
    ...
    addPropsToElement(homeworkNameDiv, {
        "class": "HomeworkText",
        "id": "divName" + idList
    }, name);
    //delete button
    const deleteButton = document.createElement("button");
    addPropsToElement(deleteButton, {
        "class": "HomeworkBtn",
        "id": idList + "dlt"
    }, "Eliminar");
    //ModifyButton
    const modifyButton = document.createElement("button");
    addPropsToElement(modifyButton, {
        "class": "HomeworkBtn",
        "id": idList + "mdf"
    }, "Modificar");
    //Container
    const HmwkContainer = document.createElement("div");
    addPropsToElement(HmwkContainer, {
...
function addElementsToFather(Father, ...children) {
    for(let index = 0; index < children.length; index++)Father.appendChild(children[index]);
}
function addPropsToElement(element, props, ...innerHTML) {
    if (innerHTML[0] != undefined) element.innerHTML = innerHTML[0];
    for(let property in props)element.setAttribute(`${property}`, `${props[property]}`);
}
...
function showItemsOnClick(divID) {
    ...
    if (homework != 2) {
        ...
        selectedHomework.innerHTML = "nombre: " + homework.name + " , fecha inicio: " + homework.dateInit + " , fecha fin: " + homework.dateFin + " , materia: " + homework.courseName + ", Horas necesarias:  " + homework.hoursNeeded + ", Horas por dia:  " + homework.getHoursPerDay() + ", feedback de horas:  " + homework.sumFeedback();
    } else selectedHomework.innerHTML = "";
}
...
},{"./errorCodes":"7XpT9","./coursesController":"fT3YC","./studentList.js":"2nFcC","./TeacherLoginManager":"35fYT","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["62ocA","9Qpox"], "9Qpox", "parcelRequirec771")

//# sourceMappingURL=index.88f43d0c.js.map
